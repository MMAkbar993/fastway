...
const title = document.querySelector("#standing-title");
const btnA = document.querySelector("#btn-a");
const btnB = document.querySelector("#btn-b");

if (serieA && serieB && btnA && btnB) {
    btnA.addEventListener("click", () => {
        if (title) title.textContent = "FASTWAY A serie standing";
    ...
    btnB.addEventListener("click", () => {
        if (title) title.textContent = "FASTWAY B serie standing";
...
// Playoff page series switching
const playoffContentA = document.getElementById("playoffContentA");
const playoffContentB = document.getElementById("playoffContentB");
const playoffTitle = document.querySelector("#playoff-title");
const playoffBtnA = document.querySelector("#btn-a");
const playoffBtnB = document.querySelector("#btn-b");

if (playoffContentA && playoffContentB && playoffTitle && playoffBtnA && playoffBtnB) {
    // Check if we're on the playoff page
    if (window.location.pathname.includes('playoff.html') || document.querySelector('.playoff-content')) {
        playoffBtnA.addEventListener("click", () => {
            playoffTitle.textContent = "FASTWAY A serie playoff";
            playoffContentA.classList.remove("hidden");
            playoffContentB.classList.add("hidden");
            playoffBtnA.classList.add("active");
            playoffBtnB.classList.remove("active");
        });

        playoffBtnB.addEventListener("click", () => {
            playoffTitle.textContent = "FASTWAY B serie playoff";
            playoffContentA.classList.add("hidden");
            playoffContentB.classList.remove("hidden");
            playoffBtnB.classList.add("active");
            playoffBtnA.classList.remove("active");
        });

        // Initialize with Serie A
        if (window.onload) {
...
// Prevent logo from doing anything
...
function updateMainTable(skipWinPct = true) {
    try {
        ...
        rows.forEach((row) => {
            ...
            const wins = parseInt(getText(COL_WIN)) || 0;
...
// Structure des rounds (use global if already defined in playoff.js, otherwise define it)
...
function handlePlayoffProgression() {
    ...
    saveRaceState(); // Save updated playoff state
...
function updateRaceCounterUI() {
    ...
    if (counterEl) {
        ...
        counterEl.innerHTML = `
    <span style="color:#fff; font-size:15px; margin-right:5px;">${raceLabelText}</span>
    <span style="color:#fff; font-size:16px;">${currentRace}</span>
    <span style="color:#fff; margin:0 1px; font-size:16px;">/</span> 
    <span style="color:#fff; font-size:16px;">${maxRaces}</span>
`;
    }
}
...
function saveRaceState() {
    ...
    // Also save playoff state and data separately for playoff page
    localStorage.setItem('fastway_playoff_state', JSON.stringify(playoffState));
    localStorage.setItem('fastway_playoff_data', JSON.stringify(playoffData));
}
...
function loadRaceState() {
    try {
        ...
        // Load playoff state and data
        if (s.playoffState) playoffState = s.playoffState;
        if (s.playoffData) playoffData = s.playoffData;

        // Also try to load from separate keys (for playoff page compatibility)
        try {
            const savedPlayoffState = localStorage.getItem('fastway_playoff_state');
            if (savedPlayoffState) {
                const parsed = JSON.parse(savedPlayoffState);
                playoffState = { ...playoffState, ...parsed };
            }
            const savedPlayoffData = localStorage.getItem('fastway_playoff_data');
            if (savedPlayoffData) {
                const parsed = JSON.parse(savedPlayoffData);
                playoffData = { ...playoffData, ...parsed };
            }
        } catch (e) {
            console.warn('Error loading playoff state/data:', e);
        }
...
function getSerieRows() {
    ...
    if (!targetSerie) return [];
    return Array.from(targetSerie.querySelectorAll(".column-row"));
}

function getAllSerieRows() {
    const rowsA = serieA ? serieA.querySelectorAll('.column-row') : [];
    const rowsB = serieB ? serieB.querySelectorAll('.column-row') : [];
    return [...rowsA, ...rowsB];
}
...
function incrementWinColumns(row, delta = 1) {
    if (!row) {
        console.warn('incrementWinColumns: row is null or undefined');
        return;
    }

    // Déterminer la série (A ou B) à laquelle appartient cette row
    let seriesKey = 'A';
    if (serieB && serieB.contains(row)) {
        seriesKey = 'B';
    } else if (serieA && serieA.contains(row)) {
        seriesKey = 'A';
    }

    // Get the car name first
    const name = (row.children[2]?.textContent || '').trim();
    if (!name) {
        console.warn('incrementWinColumns: could not find car name in row');
        return;
    }

    // Get or create the state for this series
    let state = raceStates[seriesKey];
    if (!state) {
        // Si l'état n'existe pas, le créer
        raceStates[seriesKey] = {
            racePaused: false,
            raceRound: 1,
            raceQueue: [],
            raceWinners: [],
        ...
        state = raceStates[seriesKey];
    }

    // Initialize carStats if needed
    if (!state.carStats) state.carStats = {};
    if (!state.carStats[name]) state.carStats[name] = { wins: 0, losses: 0 };

    // Update the state first (source of truth)
    const currentWins = parseInt(state.carStats[name].wins || 0, 10) || 0;
    state.carStats[name].wins = currentWins + delta;

    // Now update the DOM to match the state
    if (typeof COL_WIN !== 'undefined' && row.children[COL_WIN]) {
        const el = row.children[COL_WIN];
        el.textContent = String(state.carStats[name].wins);
    } else {
        // Fallback: try to find by class
        const winEl = row.querySelector('.win');
        if (winEl) {
            winEl.textContent = String(state.carStats[name].wins);
        }
    }
...
function startNextDuel(duels, roundName, nextRoundName) {
    ...
    showDuelModal(car1, car2, roundName, (winner, manualTimeForWinner) => {
        const loser = winner === car1 ? car2 : car1;

        // Find winner row using the helper function
        const winnerRow = findRowByName(winner);

        // Save old best time before potential update
        let oldBestTime = '';
        if (winnerRow && winnerRow.children[6]) {
            oldBestTime = winnerRow.children[6].textContent.trim();
        }

        // Track duel in history for undo
        duelHistory.push({
            winner: winner,
            loser: loser,
            serie: raceSerieVisible,
            oldBestTime: oldBestTime,
            timestamp: Date.now()
        });
        if (duelHistory.length > 100) duelHistory.shift(); // Keep last 100
        saveDuelHistory();

        // Update win column, win%, best time, and total points
        if (winnerRow) {
            // Determine the correct series for this race
            const currentSerie = raceSerieVisible;

            // Update win column - this will update both DOM and state
            incrementWinColumns(winnerRow, 1);

            // Get the updated state to ensure we have the latest wins
            const correctState = raceStates[currentSerie];
            if (correctState) {
                // Update win percentage with the correct state
                updateWinPercentage(winnerRow, correctState);
            } else {
                updateWinPercentage(winnerRow);
            }
        } else {
            console.warn('Winner row not found for:', winner);
            // Even if row not found, still update state for the winner
            const currentSerie = raceSerieVisible;
            let state = raceStates[currentSerie];
            if (!state) {
                raceStates[currentSerie] = {
                    racePaused: false,
                    raceRound: 1,
                    raceQueue: [],
                    raceWinners: [],
                ...
                state = raceStates[currentSerie];
            }
            if (!state.carStats) state.carStats = {};
            if (!state.carStats[winner]) state.carStats[winner] = { wins: 0, losses: 0 };
            state.carStats[winner].wins = (parseInt(state.carStats[winner].wins || 0, 10) || 0) + 1;
            saveRaceState();
        }
        ...
        // Update the main table to refresh all calculations
        if (typeof updateMainTable === 'function') {
            updateMainTable(false); // false = don't skip win percentage update
        }

        // Continue with next duel
...
window.addEventListener("load", () => {
    ...
    function syncStateFromDom() {
        if (!serieA && !serieB) return; // Skip if elements don't exist
...
function initTableSorting() {
    ...
    sortableHeaders.forEach(header => {
        ...
        header.addEventListener('click', () => {
            ...
            const container = serieA.classList.contains('hidden') ? serieB : serieA;
            sortTable(container, sortType);
        });
    });
}

function sortTable(container, sortType) {
    const rows = Array.from(container.querySelectorAll('.column-row'));
    if (rows.length === 0) return;

    const isAsc = currentSort.column === sortType && currentSort.direction === 'asc';
    currentSort = { column: sortType, direction: isAsc ? 'desc' : 'asc' };
    ...
    rows.sort((a, b) => {
        ...
        switch (sortType) {
            case 'pos':
                ...
                break;
            case 'car':
                aVal = (a.children[2]?.textContent || '').trim().toLowerCase();
                bVal = (b.children[2]?.textContent || '').trim().toLowerCase();
                break;
            case 'points':
                aVal = parseFloat(a.children[3]?.textContent.replace(',', '.') || '0') || 0;
                bVal = parseFloat(b.children[3]?.textContent.replace(',', '.') || '0') || 0;
                break;
            case 'win':
                aVal = parseInt(a.children[4]?.textContent || '0') || 0;
                bVal = parseInt(b.children[4]?.textContent || '0') || 0;
                break;
            case 'chwin':
                aVal = parseInt(a.children[5]?.textContent || '0') || 0;
                bVal = parseInt(b.children[5]?.textContent || '0') || 0;
                break;
            case 'besttime':
                const aTime = (a.children[6]?.textContent || '').trim();
                const bTime = (b.children[6]?.textContent || '').trim();
                aVal = aTime === '--' || aTime === 'DNF' ? Infinity : parseFloat(aTime.replace(',', '.')) || Infinity;
                bVal = bTime === '--' || bTime === 'DNF' ? Infinity : parseFloat(bTime.replace(',', '.')) || Infinity;
                break;
            case 'winpct':
                const aPct = (a.children[7]?.textContent || '').replace('%', '').trim();
                const bPct = (b.children[7]?.textContent || '').replace('%', '').trim();
                aVal = aPct === '--' ? 0 : parseFloat(aPct) || 0;
                bVal = bPct === '--' ? 0 : parseFloat(bPct) || 0;
                break;
            case 'total':
                aVal = parseInt(a.children[8]?.textContent || '0') || 0;
                bVal = parseInt(b.children[8]?.textContent || '0') || 0;
                break;
            default:
                return 0;
        }
    ...
    rows.forEach((row, index) => {
        row.children[0].textContent = index + 1; // Update position
        container.appendChild(row);
    });
}

/* ---------- Car Click Functionality ---------- */
function initCarClickHandlers() {
    if (!serieA && !serieB) return; // Skip if elements don't exist
    const allRows = getAllSerieRows();
    allRows.forEach(row => {
        ...
        if (carCell) {
            ...
            carCell.style.textDecoration = 'underline';
            carCell.style.textDecorationColor = 'rgba(0, 231, 255, 0.5)';
            carCell.addEventListener('click', () => {
                const carName = carCell.textContent.trim();
                if (carName) {
                    window.location.href = `driverdetail.html?driver=${encodeURIComponent(carName)}`;
                }
            });
        }
    });
